...
void gazebo::GazeboRosImuSensor::OnUpdate()
{
  ...
  if (imu_data_publisher.getNumSubscribers() > 0)
  {
    ...
    ignition::math::Quaterniond orientation = sensor->Orientation();

    imu_msg.orientation.x = orientation.X() + GaussianKernel(0,0);
    imu_msg.orientation.y = orientation.Y() + GaussianKernel(0,0);
    imu_msg.orientation.z = orientation.Z() + GaussianKernel(0,0);
    imu_msg.orientation.w = orientation.W() + GaussianKernel(0,0);
