...
void GazeboRosP3D::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  // calculate offset
  double world_lat, world_lon, ned_lat, ned_lon;
  this->rosnode_->param<double>("world_lat", world_lat, 0);
  this->rosnode_->param<double>("world_lon", world_lon, 0);
  this->rosnode_->param<double>("ned_lat", ned_lat, 0);
  this->rosnode_->param<double>("ned_lon", ned_lon, 0);
  double x, y;
  convert_global_to_local_coords(ned_lat, ned_lon, world_lat, world_lon, x, y);
  this->offset_.Pos() = ignition::math::Vector3d(x, -y, 0);
  ROS_INFO_STREAM("Offset: x = " << x << ", y = " << -y);
