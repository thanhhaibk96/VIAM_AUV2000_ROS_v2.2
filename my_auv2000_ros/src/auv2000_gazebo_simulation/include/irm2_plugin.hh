#ifndef IRM2_PLUGIN_HH
#define IRM2_PLUGIN_HH
...
namespace gazebo
{
  // Foward declaration of AUVIRM2 class
  class AUVIRM2;
  ...
  class IRM2
  {
    ...
    public: explicit IRM2(AUVIRM2 *_parent);
    ...
    public: physics::JointPtr irm2Joint;

    /// \brief Plugin parent pointer - for accessing world, etc.
    protected: AUVIRM2 *plugin;
  };

  class AUVIRM2 : public ModelPlugin
  {
    /// \brief Constructor.
    public: AUVIRM2() = default;

    /// \brief Destructor.
    public: virtual ~AUVIRM2() = default;
    ...
    public: bool onSetIRM2CallBack(utils::CommandLongRequest &_req,
                                     utils::CommandLongResponse &_res);
    public: void OnIRM2CmdCallBack(const utils::anti_rolling_stamped::ConstPtr &_msg);
    public: void OnIRM2CmdKeyboardCallBack(const utils::KeyboardCommand::ConstPtr &_msg);
    ...
    public: std::string setIRM2Service;

    /// \brief Subscription to GCS commands.
    public: ros::ServiceServer resSetIRM2;
    public:ros::Publisher pubIRM2;
    /// \brief IRM2-locking status from GCS.
    public: bool IRM2Locked = true;

    /// \brief Keyboard control status from keyboard.
    public: bool keyboardEnabled = true;

    /// \brief Vector of thruster instances
    private: std::vector<IRM2> irm2;
