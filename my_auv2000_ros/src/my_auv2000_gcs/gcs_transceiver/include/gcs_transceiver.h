...
private:
  ...
  /**
   * @brief Manager to handle joystick microservice.
   */
  Joystick_Protocol* joystickProtocol = nullptr;
  ...
  /**
     ...
     */
    ros::Subscriber subGPS_DVL_INS;

    /**
     * @brief Callback for GPS subscription.
     * @param[in] msg Raw GPS message.
     */
    void onGPS_DVL_INSCallBack(const gps_dvl_ins_stamped::ConstPtr& msg);

  /**
   * @brief ROS Subscriber to My_AUV2000's Data.
   */
  ros::Subscriber subMass_Shifter;
  ros::Subscriber subPistol;
  ros::Subscriber subThruster;
  ros::Subscriber subBoardARM1;
  ros::Subscriber subBoardARM2;

  /**
   * @brief Callback for My_AUV2000's Data subscription.
   * @param[in] msg My_AUV2000's message.
   */
  void onGetMass_ShifterCallBack(const motor_stamped::ConstPtr& msg);
  void onGetPistolCallBack(const motor_stamped::ConstPtr& msg);
  void onGetThrusterCallBack(const motor_stamped::ConstPtr& msg);
  void onGetBoardARM1CallBack(const board_arm1_stamped::ConstPtr& msg);
  void onGetBoardARM2CallBack(const board_arm2_stamped::ConstPtr& msg);
